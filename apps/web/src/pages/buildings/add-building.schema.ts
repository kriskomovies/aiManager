...
const requiredNumberField = (fieldName: string, min: number = 0, max?: number, isInteger: boolean = false) =>
  z.preprocess(
    (val) => {
    ...
    .max(max || Number.MAX_SAFE_INTEGER, max ? `${fieldName} не може да бъде повече от ${max}` : '')
...
export const addBuildingSchema = z.object({
  ...
  taxGenerationPeriod: z.nativeEnum(TaxGenerationPeriod, {
    errorMap: () => ({ message: 'Моля изберете валиден период за генериране на такси' })
  }),
  
  taxGenerationDay: requiredNumberField('Денят за генериране на такси', 1, 31, true),
  
  homebookStartDate: z.string()
  ...
  peopleWithAccess: z.array(z.string())
