...
import { useCreateMonthlyFeeMutation } from '@/redux/services/monthly-fee.service';
import {
  FeePaymentBasis,
  FeeApplicationMode,
...
export function CreateMonthlyFeeModal() {
  ...
  const [createMonthlyFee, { isLoading: isCreating }] =
  ...
  // Initialize apartments when data is loaded
  useEffect(() => {
    if (apartments.length > 0 && formData.apartments.length === 0) {
      setFormData(prev => ({
        ...prev,
  ...
  }, [apartments, formData.apartments.length]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const createData: ICreateMonthlyFeeRequest = {
      ...
      await createMonthlyFee(createData).unwrap();

      dispatch(
        addAlert({
          type: 'success',
          title: 'Успешно създаване!',
          message: `Месечната такса "${formData.name}" беше създадена успешно.`,
          duration: 5000,
        })
      );
      ...
      dispatch(
        addAlert({
          ...
          message:
            'Възникна грешка при създаването на месечната такса. Моля опитайте отново.',
          duration: 5000,
        })
      );
    }
