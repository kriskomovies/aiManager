...
export function CashierPaymentModal() {
  ...
  const [paymentMode, setPaymentMode] = useState<'pay-all' | 'manual'>(
    'pay-all'
  );
  ...
  const subscriptionNumber =
    (modalData?.subscriptionNumber as string) || '123456';
  ...
  const handlePayment = () => {
    ...
    const paymentData = {
      ...
      paymentMode,
      amount:
        paymentMode === 'pay-all' ? totalAmount : calculateTotalManualAmount(),
  ...
  const calculateTotalManualAmount = () => {
    return Object.values(feePaymentAmounts).reduce(
      ...
      <div className="p-6 space-y-6 overflow-y-auto max-h-[calc(90vh-180px)]">
        ...
        <div>
          ...
          >
            ...
            {residents.map(
              (resident: { id: string; name: string; surname: string }) => (
              ...
              <tbody>
                {fees.map(fee => (
                  <tr key={fee.id} className="border-t border-gray-100">
                    ...
                    <td className="py-3 px-4 text-gray-600">Януари 2025</td>
                    ...
                    {paymentMode === 'manual' && (
                      <td className="py-3 px-4 text-right">
                        <Input
                          ...
                          onChange={e =>
                            handleFeeAmountChange(fee.id, e.target.value)
                          }
